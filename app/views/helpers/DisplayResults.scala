...
object Table {
      ...
      (parseColumns(headersString),
        ...
        case (ColsParser.Success(headers, _), ColsParser.Success(columnNames, _), parsedLinks) =>
          ...
          val links: Map[String, Link] = parsedLinks match {
            case Some(LinksParser.Success(result, _)) => result.map {
              l => (l.column, l)
            }.toMap
            case _ => Map()
          }
  ...
  def table(sql: SqlStmt, headersString: String, columnsString: String, withClass: String = null, withLinks: String = null): Html =
    table(sql, headersString, columnsString,
      withClass match {
        case null => None
        case _ => Some(withClass)
      },
      withLinks match {
        case null => None
        case _ => Some(withLinks)
      }
    )

  // Public exposed function, display result of a sql query with default values
  def table(sql: SqlStmt, details: Option[(String, String)]): Html = {
    val headersString = "[" + sql.getTitles().mkString("|") + details.map(theDetail => "|Détail]").getOrElse("]")
    val columnsString = "[" + sql.columns().mkString("|") + details.map(theDetail => "|Afficher détail]").getOrElse("]")
    val detailString = details.map {
      theDetail =>
        "[Afficher détail->" + theDetail._2 + "(" + theDetail._1 + ")]"
    }.getOrElse(null)
    table(sql, headersString, columnsString, "table table-bordered", detailString)
  }
  ...
  private def tableBody(columns: List[Column], links: Map[String, Link], data: List[Map[String, Any]]) =
    "<tbody>" +
      data.foldLeft("") {
        (acc, line) => acc + tableLine(columns, links, line)
      } +
      "</tbody>"
  ...
  private object Parsers {
    ...
    object ColsParser extends RegexParsers {
      def start: Parser[String] = """\[""".r

      // Start with [
      def end: Parser[String] = """\]""".r
